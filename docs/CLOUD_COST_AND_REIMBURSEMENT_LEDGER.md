...
| Tencent COS | 香港；私有桶 `level-grind-research-hk-1460544513` | 已启用；Single-AZ、Private Read/Write、版本控制、SSE-COS；生产域名和本地预览 CORS 已配置 | 新用户 50 GB / 180 天免费包；开通无预付订单金额 | 免费包外按存储容量、请求和出网流量按量计费；版本历史会增加容量 | 每月 25 日；免费包到期前 30 日 | 核对容量、版本历史与外网流量；正式上传前完成生命周期和两账户读回测试 | COS 用量、费用明细、Invoice、Bucket 配置截图 |
...
- **2026-08-05：已充值 USD 20。** 该金额是账户充值而非 COS 单项费用；报销时应同时保存 Payment Record，并以月度资源账单分配实际用途。
...
- COS 保持私有访问和 SSE-COS；正式上传前补齐生命周期规则、最小权限服务身份和端到端完整性验证。
