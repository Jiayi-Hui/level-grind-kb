...
test("worker is portable, serialised, idempotent and records refresh lineage", async () => {
  const [worker, migration] = await Promise.all([readFile(new URL("../services/tencent-notes-api/price-refresh-worker.mjs", import.meta.url), "utf8"), readFile(new URL("../infra/shared-data/postgres/002_claim_price_refresh.sql", import.meta.url), "utf8")]);
  assert.match(worker, /pg_try_advisory_lock/); assert.match(worker, /ON CONFLICT \(series_id,trading_date\)/); assert.match(worker, /ON CONFLICT \(claim_id,series_id,calculation_version\)/); assert.match(worker, /price_refresh_failures/); assert.match(worker, /retries = 3/); assert.match(migration, /price_refresh_runs/); assert.match(migration, /source_updated_at/);
});
