...
function noteMeta(row) { return { id: row.id, title: row.title, sourceKind: row.source_kind, sensitivityLevel: row.sensitivity_level, aiProcessingAllowed: row.ai_processing_allowed, externalSearchAllowed: row.external_search_allowed, downloadAllowed: row.download_allowed, version: row.version, createdAt: row.created_at, updatedAt: row.updated_at, owner: { display_name: row.owner_name, email: row.owner_email } }; }
function ideaMeta(row) { return { id: row.id, title: row.title, ticker: row.ticker, direction: row.direction, status: row.status, noteIds: row.note_ids || [], noteTitles: row.note_titles || [], version: row.version, createdAt: row.created_at, updatedAt: row.updated_at, owner: { display_name: row.owner_name, email: row.owner_email } }; }
async function listEntity(type, actor) {
  ...
  return result.rows.map(type === "note" ? noteMeta : ideaMeta);
}
function normal(type, raw, operation) {
  ...
  if (type === "note") return { id, title, text, expectedVersion, sourceKind: clean(raw.sourceKind || "manual_note", 80).replace(/[^a-z0-9_-]/gi, "_") || "manual_note", sensitivityLevel: ["public","internal","confidential","restricted"].includes(raw.sensitivityLevel) ? raw.sensitivityLevel : "internal", aiProcessingAllowed: raw.aiProcessingAllowed === true, externalSearchAllowed: raw.externalSearchAllowed === true, downloadAllowed: raw.downloadAllowed === true };
  ...
  return { id, title, text, expectedVersion, ticker: clean(raw.ticker, 32).toUpperCase(), noteIds, direction: ["long","short","watch"].includes(raw.direction) ? raw.direction : "watch", status: ["draft","pending_review","approved","rejected","archived"].includes(raw.status) ? raw.status : "draft" };
}
...
async function mutateEntity(type,actor,input,operation) { const client=await pool.connect(); try { await client.query("BEGIN"); const table=type === "note"?"research_notes":"research_ideas"; const row=(await client.query(`SELECT * FROM ${table} WHERE id=$1 AND team_id=$2 AND deleted_at IS NULL FOR UPDATE`,[input.id,TEAM])).rows[0]; if(!row)throw err(`${type.toUpperCase()}_NOT_FOUND`,404); if(row.version!==input.expectedVersion)throw err("VERSION_CONFLICT",409,{currentVersion:row.version}); if(!roleCanEdit(actor,row))throw err("EDIT_FORBIDDEN",403); let next;
    ...
    else if(type === "note") {const e=encrypted("body",input.text,type,input.id); next=(await client.query(`UPDATE research_notes SET title=$2,body_ciphertext_b64=$3,body_nonce_b64=$4,body_auth_tag_b64=$5,body_wrapped_data_key_b64=$6,body_key_wrap_nonce_b64=$7,body_key_wrap_auth_tag_b64=$8,body_key_version=$9,source_kind=$10,sensitivity_level=$11,ai_processing_allowed=$12,external_search_allowed=$13,download_allowed=$14,version=version+1,updated_at=now() WHERE id=$1 RETURNING *`,[input.id,input.title,e.body_ciphertext_b64,e.body_nonce_b64,e.body_auth_tag_b64,e.body_wrapped_data_key_b64,e.body_key_wrap_nonce_b64,e.body_key_wrap_auth_tag_b64,e.body_key_version,input.sourceKind,input.sensitivityLevel,input.aiProcessingAllowed,input.externalSearchAllowed,input.downloadAllowed])).rows[0];}
    else {if(input.status !== row.status && !["draft", "pending_review"].includes(input.status) && !roleCanReview(actor)) throw err("IDEA_REVIEW_FORBIDDEN",403); const e=encrypted("thesis",input.text,type,input.id); next=(await client.query(`UPDATE research_ideas SET title=$2,ticker=$3,direction=$4,status=$5,thesis_ciphertext_b64=$6,thesis_nonce_b64=$7,thesis_auth_tag_b64=$8,thesis_wrapped_data_key_b64=$9,thesis_key_wrap_nonce_b64=$10,thesis_key_wrap_auth_tag_b64=$11,thesis_key_version=$12,version=version+1,updated_at=now() WHERE id=$1 RETURNING *`,[input.id,input.title,input.ticker,input.direction,input.status,e.thesis_ciphertext_b64,e.thesis_nonce_b64,e.thesis_auth_tag_b64,e.thesis_wrapped_data_key_b64,e.thesis_key_wrap_nonce_b64,e.thesis_key_wrap_auth_tag_b64,e.thesis_key_version])).rows[0]; await reconcileIdeaNotes(client,input.id,input.noteIds,actor);}
...
async function recordAccess(type,id,actor,input){const table=type === "note"?"research_notes":"research_ideas"; const r=(await pool.query(`SELECT * FROM ${table} WHERE id=$1 AND team_id=$2 AND deleted_at IS NULL`,[id,TEAM])).rows[0];if(!r)throw err(`${type.toUpperCase()}_NOT_FOUND`,404); if(type!=="note")throw err("ACCESS_ACTION_UNSUPPORTED");if(input.action==="download"&&!r.download_allowed)throw err("NOTE_DOWNLOAD_FORBIDDEN",403);if(input.action==="ai_use"&&!r.ai_processing_allowed)throw err("NOTE_AI_FORBIDDEN",403);if(input.externalSearch===true&&!r.external_search_allowed)throw err("NOTE_EXTERNAL_SEARCH_FORBIDDEN",403);const c=await pool.connect();try{await c.query("BEGIN");await audit(c,actor,type,id,input.action,r.version,r.version,{externalSearch:input.externalSearch===true});await c.query("COMMIT");}catch(e){await c.query("ROLLBACK");throw e;}finally{c.release();}}
