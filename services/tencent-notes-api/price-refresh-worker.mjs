...
export function calculateWindow(claimedAt, ticker, observed, suppliedZone) {
  ...
  const base = prices[baseIndex]; const t0 = prices[t0Index]; const returns = Object.fromEntries(HORIZONS.map((horizon) => {
    ...
    return [`t${horizon}`, { date: point.date, close: Number(point.close), return: Number.isFinite(value) ? value : null, status: Number.isFinite(value) ? "observed" : "invalid" }];
  }));
...
export async function refreshClaimPrices({ databaseUrl = process.env.DATABASE_URL, fetchImpl, now = new Date().toISOString() } = {}) {
  ...
  try {
    ...
    for (const claim of claims) { try { const yahoo=await fetchYahooDaily(claim.ticker,{fetchImpl}); const window=calculateWindow(claim.claimed_at,claim.ticker,yahoo.points,claim.market_timezone || yahoo.marketTimezone); const db=await pool.connect(); try { await db.query("BEGIN"); const series=(await db.query(`INSERT INTO market_price_series (ticker,provider,market_timezone,currency,adjusted,last_observation_date,updated_at) VALUES ($1,$2,$3,$4,false,$5,now()) ON CONFLICT (ticker,provider,adjusted) DO UPDATE SET market_timezone=EXCLUDED.market_timezone,currency=EXCLUDED.currency,last_observation_date=EXCLUDED.last_observation_date,updated_at=now() RETURNING id`,[yahoo.ticker,PROVIDER,window.marketTimezone,yahoo.currency,yahoo.points.at(-1)?.date || null])).rows[0]; for(const point of yahoo.points) await db.query(`INSERT INTO market_price_points (series_id,trading_date,open,high,low,close,volume) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (series_id,trading_date) DO UPDATE SET open=EXCLUDED.open,high=EXCLUDED.high,low=EXCLUDED.low,close=EXCLUDED.close,volume=EXCLUDED.volume`,[series.id,point.date,point.open,point.high,point.low,point.close,point.volume]); if(window.baseDate) await db.query(`INSERT INTO claim_price_windows (claim_id,series_id,base_date,t0_date,base_close,returns,calculation_version,calculated_at,source_provider,source_updated_at,refresh_run_id) VALUES ($1,$2,$3,$4,$5,$6::jsonb,$7,now(),$8,$9,$10) ON CONFLICT (claim_id,series_id,calculation_version) DO UPDATE SET base_date=EXCLUDED.base_date,t0_date=EXCLUDED.t0_date,base_close=EXCLUDED.base_close,returns=EXCLUDED.returns,calculated_at=now(),source_provider=EXCLUDED.source_provider,source_updated_at=EXCLUDED.source_updated_at,refresh_run_id=EXCLUDED.refresh_run_id`,[claim.id,series.id,window.baseDate,window.t0Date,window.baseClose,JSON.stringify(window.returns),CALCULATION_VERSION,PROVIDER,yahoo.sourceUpdatedAt,runId]); await db.query("COMMIT"); refreshed += 1; } catch(error){await db.query("ROLLBACK");throw error;} finally{db.release();} } catch(error){failed +=1; await pool.query(`INSERT INTO price_refresh_failures (refresh_run_id,claim_id,error_code,attempted_at) VALUES ($1,$2,$3,now()) ON CONFLICT (refresh_run_id,claim_id) DO UPDATE SET error_code=EXCLUDED.error_code,attempted_at=now()`,[runId,claim.id,error.code || "PRICE_REFRESH_FAILED"]); } }
